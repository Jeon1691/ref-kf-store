 ...
 * @author Yuan Ji
 */
public class SocialAuthenticationFilterForKakao extends AbstractAuthenticationProcessingFilter {

    private SocialAuthenticationServiceLocator authServiceLocator;

    private String signupUrl = "/signup";

    private String signupUrlForKakao = "/signup/kakao";
    ...
    private SimpleUrlAuthenticationFailureHandlerForKakao delegateAuthenticationFailureHandler;

    private SessionStrategy sessionStrategy = new HttpSessionSessionStrategy();

    private String filterProcessesUrl = DEFAULT_FILTER_PROCESSES_URL;

    public SocialAuthenticationFilterForKakao( AuthenticationManager authManager, UserIdSource userIdSource, UsersConnectionRepository usersConnectionRepository, SocialAuthenticationServiceLocator authServiceLocator ) {
        super( DEFAULT_FILTER_PROCESSES_URL );
        setAuthenticationManager( authManager );
        this.userIdSource = userIdSource;
        this.usersConnectionRepository = usersConnectionRepository;
        this.authServiceLocator = authServiceLocator;
        this.delegateAuthenticationFailureHandler = new SimpleUrlAuthenticationFailureHandlerForKakao( DEFAULT_FAILURE_URL );
    ...
    public void setPostFailureUrl( String postFailureUrl ) {
        ...
        if ( failureHandler instanceof SimpleUrlAuthenticationFailureHandlerForKakao ) {
            SimpleUrlAuthenticationFailureHandlerForKakao h = (SimpleUrlAuthenticationFailureHandlerForKakao) failureHandler;
    ...
    public Authentication attemptAuthentication( HttpServletRequest request, HttpServletResponse response ) throws AuthenticationException {
        ...
        if ( StringUtils.isNotEmpty( kakaoAgent ) ) {
            request.getSession().setAttribute( KakaoFriendsConstants.KAKAO_WEB_VIEW, KakaoFriendsConstants.KAKAO_WEB_VIEW_4TAB );
        }

        String redirectUrl = request.getParameter( "redirect_url" );

        if ( StringUtils.isNotEmpty( redirectUrl ) ) {
            request.getSession().setAttribute( KakaoFriendsConstants.KAKAO_AUTH_AFTER_REDIRECT_URL, redirectUrl );
        }
    ...
    protected boolean detectRejection( HttpServletRequest request ) {
        ...
        if ( parameterKeys.size() == 1 && parameterKeys.contains( KakaoFriendsConstants.KAKAO_DISABLE_APPROVAL_WINDOW ) ) {
            return false;
        }

        if ( parameterKeys.size() == 2 && parameterKeys.contains( KakaoFriendsConstants.KAKAO_DISABLE_APPROVAL_WINDOW ) && parameterKeys.contains( "redirect_url" ) ) {
            return false;
        }
    ...
    private Authentication doAuthentication( SocialAuthenticationService<?> authService, HttpServletRequest request, SocialAuthenticationToken token ) {
        try {
            ...
            request.getSession().removeAttribute( KakaoFriendsConstants.KAKAO_SSO_MODE );
    ...
    private String buildSignupUrl( HttpServletRequest request ) {
        ...
        int webViewMode = (int) request.getSession().getAttribute( KakaoFriendsConstants.KAKAO_WEB_VIEW );

        String url = webViewMode == KakaoFriendsConstants.KAKAO_WEB_VIEW_4TAB ? signupUrlForKakao : signupUrl;

        if ( url.startsWith( "http://" ) || url.startsWith( "https://" ) ) {
            return url;
        }
        if ( !url.startsWith( "/" ) ) {
            return ServletUriComponentsBuilder.fromContextPath( request ).path( "/" + url ).build().toUriString();
        }
        return ServletUriComponentsBuilder.fromContextPath( request ).path( url ).build().toUriString();
    }
