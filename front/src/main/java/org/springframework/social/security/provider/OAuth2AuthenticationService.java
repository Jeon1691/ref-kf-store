...
public class OAuth2AuthenticationService<S> extends AbstractSocialAuthenticationService<S> {
    ...
    public SocialAuthenticationToken getAuthToken( HttpServletRequest request, HttpServletResponse response ) throws SocialAuthenticationRedirectException {
        ...
        if ( !StringUtils.hasText( code ) ) {
            ...
            if ( request.getSession() != null && request.getSession().getAttribute( KakaoFriendsConstants.KAKAO_WEB_VIEW ) != null ) {
                int kakaoWebView = (int) request.getSession().getAttribute( KakaoFriendsConstants.KAKAO_WEB_VIEW );

                String disableApprovalWindow = request.getParameter( KakaoFriendsConstants.KAKAO_DISABLE_APPROVAL_WINDOW ) != null ? request.getParameter( KakaoFriendsConstants.KAKAO_DISABLE_APPROVAL_WINDOW ) : "true";

                if ( kakaoWebView == KakaoFriendsConstants.KAKAO_WEB_VIEW_4TAB ) {
                    params.add( KakaoFriendsConstants.KAKAO_DISABLE_APPROVAL_WINDOW, disableApprovalWindow );
                }
            }
