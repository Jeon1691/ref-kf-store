<template>
  <div class="change_delivery">
    <form ref="formShipping"
      ...
      <form-hidden :omNum="orderInfo.omNum"
                   :orderAddressRequest="orderAddressRequest"></form-hidden>
      ...
      <fieldset>
        ...
        <dl class="list_delivery">
          ...
          <dt>
            <label for="orderPhone">{{ $t('shipping_address.phone') }}</label>
            <span class="ico_friends ico_star">{{ $t('input_necessary') }}</span>
          </dt>
          <dd>
            <div class="box_input">
              <input type="text"
                     ...
                     id="orderPhone"
                     class="tf_g"
                     :placeholder="$t('shipping_address.phone')"
                     v-model="oaPhone">
            </div>
          </dd>
          ...
          <dd class="subject_postal">
            ...
            <button v-if="!globalOrder"
                    type="button"
                    class="btn_postal"
                    @click="handleModal">{{ $t('shipping_address.postal') }}</button>
          </dd>
          ...
          <dd v-if="orderAddressRequest.oaCountryCode === 'US'"
              class="wrap_opt">
            <div class="opt_sort"
              ...
              <div class="box_sort">
                ...
                <ul class="list_sort">
                  <li v-for="usState in usStateList"
                      :key="usState.CODE">
                    <a @click="changeState(`${usState.CODE} (${usState.STATE_EN})`)"
          ...
          <dd v-else>
            <div class="box_input">
              <input type="text"
                     ...
                     id="orderAddress1"
                     class="tf_g tf_on"
                     :placeholder="globalOrder ? $t('shipping_address.write_english') : $t('shipping_address.addr1')"
                     v-model="oaAddr1"
                     :disabled="!globalOrder">
            </div>
          </dd>
          ...
          <dd>
            <div class="box_input">
              <input type="text"
                     ...
                     id="orderAddress2"
                     class="tf_g"
                     :placeholder="globalOrder ? $t('shipping_address.write_english') : $t('shipping_address.addr2')"
                     v-model="oaAddr2">
            </div>
          </dd>
          ...
          <dd>
            <div class="box_input box_textarea">
              <textarea name="oaDeliMemo"
                        ...
                        class="tf_g"
                        v-model="oaDeliMemo"
                        :placeholder="!globalOrder ? $t('shipping_address.max_warning') : ''"></textarea>
            </div>
            <p class="note_info">
...
export default {
  ...
  methods: {
    ...mapMutations(['CHANGE_ORDER_ADDRESS_REQUEST', 'SHOW_MODAL', 'HIDE_MODAL', 'SHOW_DIALOG']),
    onSubmit() {
      if (this.$v.$invalid) {
        this.handleAlert(
          [
